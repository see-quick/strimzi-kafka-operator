...
public class MockCruiseControl {
     ...
     */
    private static File createKeystoreFromPem(File keyFile, File certFile) throws Exception {
        // Read and parse the CA private key
        ...
        try (PEMParser pemParser = new PEMParser(new StringReader(certPem))) {
            ...
            if (certObject instanceof X509CertificateHolder caCertHolder) {
                caCertificate = new JcaX509CertificateConverter().getCertificate(caCertHolder);
                // Get issuer directly from BouncyCastle to avoid X500Name format issues
                issuer = caCertHolder.getSubject();
            } else {
        ...
        // Generate a server certificate with localhost as SAN, signed by the CA
        ...
        X509v3CertificateBuilder certBuilder = new JcaX509v3CertificateBuilder(
                issuer, serialNumber, notBefore, notAfter, subject, serverKeyPair.getPublic());
        ...
        X509Certificate serverCertificate = new JcaX509CertificateConverter().getCertificate(certHolder);

        // Create PKCS12 keystore with server cert + CA cert chain
        KeyStore keyStore = KeyStore.getInstance("PKCS12");
        keyStore.load(null, null);
        keyStore.setKeyEntry("server", serverKeyPair.getPrivate(), KEYSTORE_PASSWORD.toCharArray(),
                new X509Certificate[]{serverCertificate, caCertificate});

        // Write keystore to temp file
        File keystoreFile = File.createTempFile("wiremock-keystore", ".p12");
